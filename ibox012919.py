...
#-------------------------------------------------------------------------------
    ...
    def experimentrun(self):
        ...
        if ((int(stage) == 11 or int(stage) == 12 or int(stage) == 21 or int(stage) == 22 or int(stage) == 23 or int(stage) == 24 or int(stage) == 6 and num % 2 == 0) or int(stage) == 1): #Ensures that stage 2 and 3 saves properly
...
# Port number notes: Front left = 150, Front right = 15, Back left = 200, Back right = 20.
        ...
        for x in range(0,trialnum):
            if stagenum == '11':
                ...
                while Trig != 15 and Trig != 200 and Trig != 30: #and Trig != 20 and Trig != 150 and Trig != 200 and Trig != 30:
                    Trig = 0
        ...
        # Stage-1 Conditon-2 (coded as stage 12) Changes port that turns off active air to front right and back left (insted of front left and back right).
            # 200-BL (back left) and 15-FR (front right) ports for self. Friend port is 20-FL (front left).
            if stagenum == '12':
                #Controls the display for the trial counter
                trialcount=trialcount+1
                trialdisplay = "{0}".format(trialcount)
                ...
                while(not IRsensorque.empty()):
                    CLOSE = IRsensorque.get()
                #Actual trial. Continually searches for signal until recieved.
                while Trig != 150 and Trig != 20 and Trig != 30: #and Trig != 20 and Trig != 150 and Trig != 200 and Trig != 30:
                    Trig = 0
                    if sensorthread.is_alive():
                        if(not IRsensorque.empty()):
                            Trig = IRsensorque.get()
            ...
            if stagenum == '31':
                ...
                if TrialId[x] == 1:
                    ...
                    while(not IRsensorque.empty()):
                        CLOSE = IRsensorque.get()
                    while Trig != 200 and Trig!= 15 and Trig != 30:
                        Trig = 0
                        if sensorthread.is_alive():
                            if(not IRsensorque.empty()):
                                Trig = IRsensorque.get()
